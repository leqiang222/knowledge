...
// 元祖作为返回值
func enableLogin() -> (result: Bool, message: String)! {
	return (true, "success")
}
	```
...
// 可以在 `for-in` 循环中使用 `where` 语句来添加过滤条件。
let array = [1, 2, 3, 4, 5]
...
`enumerated` 方法：用于在遍历数组时获得元素的索引和值。
```swift
let array = ["a", "b", "c"]
...
let drink = Beverage.coffee("Espresso")
...
```swift
// 参数标签：在参数名称前可以自定义参数标签，或者使用 `_` 表示不使用参数标签。
...
- `private：`修饰的属性或者方法只能在当前类且在一个 extension内才可访问。
...
### @discardableResult
方法如果有返回值，调用时必须有一个接收方，否则编译器警告。`@discardableResult`作用就是消除警告的。

```swift
// 如果不用@discardableResult, 你也可以使用通配符
func getTestResult() {
	return "x"				  
}
// 加通配符也不会报警告
...
### @objcMembers
用于类声明中，指示编译器自动将该类及其所有成员（包括属性和方法）暴露给Objective-C运行时，而不需要为每个成员单独添加`@objc`
