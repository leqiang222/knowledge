...
- UIApplication会从事件队列中取出最前面的事件，并将事件给应用程序的主窗口（keyWindow）
- 主窗口会在视图层次结构一层一层去找，知道`找到一个最合适的视图`来处理触摸事件。
...
- 3.若返回YES, 则向当前视图的所有子视图发送`hitTest:withEvent:`消息，（所有子视图的遍历顺序是从top到bottom，即从subviews数组 的末尾向前遍历,直到有子视图返回非空对象或者全部子视图遍历完毕）；
...
- 方法判断点在不在当前view上（方法调用者的坐标系上）如果返回YES，代表点在方法调用者的坐标系上;返回NO代表点不在方法调用者的坐标系上，那么方法调用者也就不能处理事件。