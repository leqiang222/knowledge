...
# `-C`：为密钥添加注释，通常用邮箱地址。
# `-f /Users/helloworld/.ssh/gitee_id_rsa`: 公钥四月生成路径（生成多个ssh-key需要）
ssh-keygen -t rsa -b 4096 -C "leqiang222@163.com" -f /Users/helloworld/.ssh/gitee_id_rsa
```
### 文件权限
...
sudo n [ls/list/--all]
# 查看 node 远程版本
sudo n lsr/ls-remote [--all] // 默认20个，--all展示所有
```
